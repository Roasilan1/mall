...
import QRCode from 'qrcode';
  ...
  export default {
    ...
    methods:{
      getPayInfo(){
        //获取订单支付信息
        this.$api.createNative(this.orderId).then(res=>{
           this.payInfo=res.data;
           console.log(this.payInfo);
        })
      },
      async pay(){
        const code=await QRCode.toDataURL(this.payInfo.codeUrl);
        this.$alert(`<img src='${code}' />`, '请使用微信支付', {
        ...
        if(!this.timer){
          this.timer=setInterval(async () => {
            const result =await this.$api.payStatus(this.orderId);
            console.log(result);
            if(result.code===200)
            {
