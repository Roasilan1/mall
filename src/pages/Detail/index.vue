    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :defaultImg="skuInfo.skuDefaultImg" :imageList="skuInfo.skuImageList" />
          <!-- 小图列表 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr) in spuSaleAttrList" :key="attr.id">
                ...
                <dd @click="changeActive(attrValue,attr.spuSaleAttrValueList)" 
                changepirce="0" :class="{active:attrValue.isChecked==='1'}"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input @blur="mountBlur" autocomplete="off" class="itxt" v-model="mount">
                <a href="javascript:" class="plus" @click="add">+</a>
                <a href="javascript:" class="mins" @click="reduce">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addToCart(skuInfo)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
import { mapGetters } from 'vuex';
  ...
  export default {
    ...
    computed:{
      ...
      ...mapGetters(["spuSaleAttrList","skuInfo","categoryView"])
    },
    methods:{
      changeActive(attrValue,attrs){
        // attrValue是选择的属性
        // attrs是所有可以供选择的属性  只要在所有属性中找到用户选择的属性即可  传地址的？
        attrs.forEach(item=>{
      ...
      mountBlur(){
        if(isNaN(this.mount*1))
          this.mount=1;
      },
      addToCart(skuInfo){
        this.$store.dispatch("addTocart",{skuId:this.$route.params.id,skuNum:this.mount}).then(()=>{
          sessionStorage.setItem("skuInfo",JSON.stringify(skuInfo));
          // 跳转到购物车页面
          this.$router.push({path:"/addCartSuccess",query:{mount:this.mount}});
